...
const d$dashboard = defineStore({
    ...
    actions: {
        ...
        async a$countInboxStatus() {
            try {
                this.status = null;
                const { data, status } = await s$dashboard.countInboxStatus();
                this.countInboxStatus = data ?? {};
                this.status = status;
            } catch ({ error, message }) {
                this.status = false;
                throw error ?? message;
    ...
    getters: {
        ...
        g$countInboxStatus: ({ countInboxStatus }) => countInboxStatus,
    },
});
