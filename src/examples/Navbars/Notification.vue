<template>
    ...
    <ul class="px-1 py-2 dropdown-menu dropdown-menu-end me-n4" :class="showMenu ? 'show' : ''" aria-labelledby="dropdownMenuButton" v-if="g$listNotification">
        ...
        <li class="mb-2 d-lg-block d-md-block d-none" v-for="data in g$listNotification">
            <!-- PO, status belum dibaca -->
            <a class="dropdown-item border-radius-md bg-gray-200" v-if="(data.status === 0 && data.d_notification_object.entity_type === 1)"
                :href="$router.resolve({ name: 'Pesanan Masuk Detail', params: { id: data.d_notification_object.entity_id } }).href" @click="readNotification(data.id_notification)">
            ...
            <!-- DN, status belum dibaca -->
            <a class="dropdown-item border-radius-md bg-gray-200" v-if="(data.status === 0 && data.d_notification_object.entity_type === 2)"
            ...
            <!-- PO, status sudah dibaca -->
            <a class="dropdown-item border-radius-md" v-if="(data.status === 1 && data.d_notification_object.entity_type === 1)"
                :href="$router.resolve({ name: 'Pesanan Masuk Detail', params: { id: data.d_notification_object.entity_id } }).href" @click="readNotification(data.id_notification)">
                <div class="py-1 d-flex">
                    <div class="my-auto me-3">
                        <i class="fas fa-cart-plus text-lg text-success cursor-pointer"></i>
            ...
            <!-- DN, status sudah dibaca -->
            <a class="dropdown-item border-radius-md" v-if="(data.status === 1 && data.d_notification_object.entity_type === 2)"
                :href="$router.resolve({ name: 'Tracking Detail', params: { id: data.d_notification_object.entity_id } }).href" @click="readNotification(data.id_notification)">
                <div class="py-1 d-flex">
                    <div class="my-auto me-3">
                        <i class="fas fa-envelope-open text-lg text-info cursor-pointer"></i>
